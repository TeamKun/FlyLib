...
import com.flylib.flylib3.gui.FGUIComponent
import com.flylib.flylib3.gui.Pos
 ...
 */
abstract class InventoryGUI(inventoryType: InventoryType, name: Component, val fl: FlyLib, height: Int) :
    FGUIComponent<InventoryGUIEntry, Int, Int, InventoryPos> {
    val inventory = when (inventoryType) {
        InventoryType.CHEST -> {
            Bukkit.createInventory(null, height * 9, name)
        }
        else -> {
            Bukkit.createInventory(null, inventoryType, name)
        }
    }
