...
import com.flylib.flylib3.command.argument.TypeMatcher
import com.flylib.flylib3.event.ex.FCommandEvent
import com.flylib.flylib3.util.allIndexed
import com.flylib.flylib3.util.error
import com.flylib.flylib3.util.log
...
class FCommandBuilder(
    ...
    fun <T : Any> part(
        ...
        lazyTabCompleter: ((T) -> String) = { it.toString() },
        lambda: FCommandBuilderPart<T>.() -> Unit
    ) {
        val root =
            FCommandBuilderPart<T>(null, flyLib, lazyValues, lazyParser, lazyTabCompleter, lazyMatcher, this, type)
        lambda(root)
    }
...
class FCommandBuilderPart<T : Any>(
    ...
    val lazyTabCompleter: ((T) -> String) = { it.toString() },
...
class BuiltFPathCommand(
    ...
    private fun <T : Any> generateSuggest(
        fCommandBuilderPart: FCommandBuilderPart<T>,
        sender: CommandSender,
        command: Command,
        alias: String,
        args: Array<out String>
    ): MutableList<String> {
        return generateSuggest(
            fCommandBuilderPart,
            fCommandBuilderPart.lazyValues(sender, command, alias, args),
            fCommandBuilderPart.lazyTabCompleter as (Any) -> String
        )
    }
    ...
    private fun <T : Any> generateSuggest(
        fCommandBuilderPart: FCommandBuilderPart<T>,
        values: List<*>,
        completer: (Any) -> String
    ): MutableList<String> {
        return (values as List<T>).map { completer(it as T) }.toMutableList()
    }
