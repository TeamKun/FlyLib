 ...
 */
class SizedFlatList<K>(val width: Int, val height: Int) : Iterable<FlatEntry<K>> {
    var flatList = FlatList<K>()
        private set

    fun set(x: Int, y: Int, t: K) {
        outCheck(x, y)
        flatList.setEntry(x, y, t)
    }

    fun get(x: Int, y: Int): FlatEntry<K>? {
        outCheck(x, y)
        return flatList.getEntry(x, y)
    }

    fun getForce(x: Int, y: Int): FlatEntry<K> {
        outCheck(x, y)
        return flatList.getEntryForce(x, y)
    }

    fun outCheck(x: Int, y: Int) {
        if (x <= 0 || y <= 0) {
            throw IndexOutOfSizeException(x, y, width, height)
        }
        if (width < x) {
            throw IndexOutOfSizeException(x, y, width, height)
        }
    ...
    class SizedFlatListIterator<K>(val list: SizedFlatList<K>) : Iterator<FlatEntry<K>> {
        ...
        private fun get(pointer: Pair<Int, Int>): FlatEntry<K>? {
            ...
            return list.get(pointer.first, pointer.second)
        }
        ...
        private fun getNextPointer(): Pair<Int, Int> {
            ...
            if (poi.first > list.width) {
                poi = Pair(1, poi.second + 1)
            }
            if (poi.second > list.height) {
                // すべてがこいつのせい
